...
class PingHubTests: XCTestCase {
    ...
    func testActionPush() throws {
        let message = try Fixture.jsonObject(fromFile: "notes-action-push.json")
    ...
    func testActionDelete() throws {
        let message = try Fixture.jsonObject(fromFile: "notes-action-delete.json")
        let action = PinghubClient.Action.from(message: message)
    ...
    func testActionUnsupported() throws {
        let message = try Fixture.jsonObject(fromFile: "notes-action-unsupported.json")
        let action = PinghubClient.Action.from(message: message)
        XCTAssertNil(action)
    }
